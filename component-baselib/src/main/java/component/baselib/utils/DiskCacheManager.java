...
/**
 * 使用方式:
 * final DiskCacheManager manager = new DiskCacheManager(context, Constants.LOGIN_USER);
 * 保存: manager.put(Constants.LOGIN_USER, bean.getResult());
 * 获取: LoginUser loginUser = manager.getSerializable(Constants.LOGIN_USER);
 */
public class DiskCacheManager {
    ...
    public DiskCacheManager(Context context, String uniqueName) {
        try {
            ...
            mDiskLruCache = DiskLruCache.open(cacheFile, MyApp.getAppVersionCode(), 1, 10 * 1024 * 1024);
        } catch (IOException e) {
            e.printStackTrace();
        }
    }
