package component.baselib.utils.window;
...
public class FloatWindowPermissionChecker {
    ...
    private static boolean checkOps() {
        try {
            Object object = MyApp.getApplication().getSystemService(Context.APP_OPS_SERVICE);
            if (object == null) {
                return false;
            }
            ...
            arrayOfObject1[2] = MyApp.getApplication().getPackageName();
    ...
    public static void askForFloatWindowPermission(final Context context) {
        new AlertDialog(context)
                ...
                .builder()
                .hideTitleLayout()
                .setMsg("此应用需要在应用设置中开启悬浮窗权限，否则无法正常使用")
    ...
    private static boolean isIntentAvailable(Intent intent, Context context) {
        return intent != null && context.getPackageManager().queryIntentActivities(intent, PackageManager
                .MATCH_DEFAULT_ONLY).size() > 0;
    }
    ...
    private static void applyCoolpadPermission(Context context) {
        ...
        intent.setClassName("com.yulong.android.seccenter", "com.yulong.android.seccenter.dataprotection.ui" +
                ".AppListActivity");
    ...
    private static void applyOppoPermission(Context context) {
        ...
        if (!startActivitySafely(intent, context)) {
            ...
            intent.setClassName("com.color.safecenter", "com.color.safecenter.permission.floatwindow" +
                    ".FloatWindowListActivity");
            if (!startActivitySafely(intent, context)) {
                intent.setAction("com.coloros.safecenter");
                intent.setClassName("com.coloros.safecenter", "com.coloros.safecenter.sysfloatwindow" +
                        ".FloatWindowListActivity");
                if (!startActivitySafely(intent, context)) {
                    showAlertToast(context);
                }
    ...
    private static void applyHuaweiPermission(Context context) {
        try {
            Intent intent = new Intent();
            intent.setFlags(Intent.FLAG_ACTIVITY_NEW_TASK);
            ComponentName comp = new ComponentName("com.huawei.systemmanager", "com.huawei.systemmanager" +
                    ".addviewmonitor.AddViewMonitorActivity");
            intent.setComponent(comp);
            if (!startActivitySafely(intent, context)) {
                comp = new ComponentName("com.huawei.systemmanager", "com.huawei.notificationmanager.ui" +
                        ".NotificationManagmentActivity");
                intent.setComponent(comp);
                context.startActivity(intent);
            }
